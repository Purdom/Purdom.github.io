...
$(function () {
  ...
  function addPeopleSource(index, link) {
    if (getTypeOf(link.link) === workType) {
      var source = (link.link[creator] || []).map(function (obj) {
        return index[obj["@id"]];
      });
      if (source.length === 0) {
        link.source = null;
  ...
  function addPeopleTarget(index, link) {
    var targets = link.link[leadTo].map(function (obj) {
      return index[obj["@id"]];
    }).map(function (obj) {
  ...
  function graphPeople(divId, triples) {
    ...
    var people = findLeadTo(triples).map(function (obj) {
      // source :: Triple
      // link   :: Triple
      // dest   :: Array Triple
    ...
    var colorF = function colorF(obj) {
      return sourceDestColor(maxSourceCount, maxDestCount, obj);
    };

    window.pg = graph;
    forceDirected($(divId), svg, graph, width, height, colorF, force);
  }
  ...
  function addBookTarget(index, triples, link) {
    var targets = [];
    link.link[leadTo].map(function (obj) {
      return index[obj["@id"]];
  ...
  function graphBooks(divId, triples) {
    ...
    var svg = d3.select(divId).append("svg").attr("width", width).attr("height", height);
    var index = indexById(triples);
    var books = findLeadTo(triples).map(function (obj) {
      return { source: null, link: obj, dest: null };
    }).map(function (obj) {
  ...
  function sourceDestColor(maxSource, maxDest, obj) {
    var saturation = 0.5 + obj.sourceCount / maxSource - obj.destCount / maxDest;
    return d3.hsl(hue, saturation, light);
  }

  function forceDirected(parent, svg, graph, width, height, color, force) {
    force.nodes(graph.nodes).links(graph.links).start();
    var link = svg.selectAll(".link").data(graph.links).enter().append("line").attr("class", "link").style("stroke-width", strokeWidth);
    var node = svg.selectAll(".node").data(graph.nodes).enter().append("circle").attr("class", "node").attr("r", radius).style("fill", color).on('mouseover', function (obj) {
      // console.log(obj);
      $(".selected", parent).text("" + obj.destCount + " ❯ " + obj.label + " ❯ " + obj.sourceCount);
    }).call(force.drag);
